				...
				<div class="layui-content">
					...
					<form class="layui-form select-ads" lay-filter="submit-content-form">
								...
								<select id="content_group" name="groupId" lay-verify="required">
			                      ...
			                      <c:if test="${!empty groups}">
			                      	<c:forEach var="g" items="${groups}">
			                      		<c:if test="${g.id == content.groupId}">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="text" name="name" lay-verify="required" placeholder="长度不超过30个汉字或者30个英文字母" autocomplete="off" class="layui-input" value="${content.name }">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">每日展示限额</label>
							<div class="layui-input-block">
								<input type="text" name="dayLimit" lay-verify="limit" placeholder="推广内容每日最大显示次数，默认不限" autocomplete="off" class="layui-input" value="${content.dayLimit }">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="text" name="totalLimit" lay-verify="limit|limitConfim" placeholder="推广内容总显示次数，默认不限" autocomplete="off" class="layui-input" value="${content.totalLimit }">
								<span class="unit">次</span>
							</div>
						</div>
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="hidden" id="weightHidden" value="${content.weight }">
								<div id="weight_range" data-from="${content.weight}"></div>
								<span class="unit">权重值越大则被推送的概率就越大</span>
							</div>
						</div>
...
<script rootURL=${qc.rootPath } staticURL=${qc.nginx } src="${qc.nginx }script/cloud/push/content.update.step1.js"></script>
</body>
